...
class CLegiMainWindow : public QMainWindow, public Ui::TestbedWindow
{
	Q_OBJECT
public:
	friend class imgVolRender;
	friend class vtkLegiInteractorStyle;

	CLegiMainWindow(QWidget* parent = 0, Qt::WindowFlags f = 0);
	...
	void draw();
	void show();
	void LoadImages(const char* prefix, int range);
